...
- each node is `{title, desc}` (desc optional, shown smaller)
- Aim for 3–6 nodes; more than ~6 gets cramped. For a picture diagram you
  already have as a file, use the `image` layout instead (not editable, but
  drops in any pre-made graphic).
